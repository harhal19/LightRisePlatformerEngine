...
namespace LightRise.Main
{
    public class Player : Character
    {
        ...
        readonly Vector2 FullSize = new Vector2(1, 2);
        readonly Vector2 CrouchSize = new Vector2(1, 1);
        ...
        public void Crouch()
        {
            if (Size != CrouchSize)
            {
                if (Size == FullSize)
                {
                    if (FloorBottom() == Map.LEFT_SHELF || FloorBottom() == Map.RIGHT_SHELF)
                    {
                        Fall();
                        isJumpDown = true;
                        return;
                ...
                if (Size.Y >= CrouchSize.Y)
                {
                    Size -= Vector2.UnitY * 0.2f;
                    //Position -= Vector2.UnitY * 0.2f;
                }
                if (Size.Y < CrouchSize.Y)
                    Size = CrouchSize;
            }
        ...
        public override void Draw(SpriteBatch surface, Camera camera)
        {
            ...
            {
                surface.Begin();
                surface.Draw(SimpleUtils.WhiteRect, new Rectangle(camera.WorldToWindow(Position.Add(0.5f) - new Vector2(Size.X / 2f, Size.Y - 1.5f)), (Size * camera.Scale.X).ToPoint()), Color.Green);
                surface.End();
            }
            else
            {
        ...
        public Player(Map world, Point position, GraphicsDevice graphicDevice) : base(world, position.ToVector2(), new Vector2(1, 2), 1f, 6f, 1f, 10f, 20f)
        {
        ...
        public void Step(StepState state)
        {
            if (!Locked)
            {
                ...
                if (!state.Keyboard.IsKeyDown(Keys.Left) && !state.Keyboard.IsKeyDown(Keys.A) && !state.Keyboard.IsKeyDown(Keys.Right) && !state.Keyboard.IsKeyDown(Keys.D))
                    Stop();
                if (state.Keyboard.IsKeyDown(Keys.Up) || state.Keyboard.IsKeyDown(Keys.W) || state.Keyboard.IsKeyDown(Keys.Space))
                    Jump();
                if (state.Keyboard.IsKeyDown(Keys.Down) || state.Keyboard.IsKeyDown(Keys.S) || state.Keyboard.IsKeyDown(Keys.LeftControl))
                    Crouch();
                else
                    StandUp();
                {
